...
import {AccountDatabase} from "./account-database.service";
import {FS} from "./fs";
import {
    AsyncSubscription,
    ChannelEntity,
    CountResult, eachPair,
    FileMetaData,
    FindOneResult,
    FindResult,
    IdInterface,
...
import {classToPlain, ClassType} from "@marcj/marshal";
import {Action, Role} from "./utils";
import {Observable, Subscription} from "rxjs";
import {convertPlainQueryToMongo, mongoToPlain, partialMongoToPlain} from "@marcj/marshal-mongo";
...
export class State {
    ...
    private sentEntities = new Map<ClassType<any>, { [id: string]: SentState }>();
    ...
    /**
     * Sends and subscribes initial available items that meet the `filter` criteria.
     * Items that become valid afterwards via update or patch are not considered here. Use findAndSubscribe instead.
     */
    @Action()
    @Role(RoleType.regular)
    findOne<T extends IdInterface>(classType: ClassType<T>, filter: { [field: string]: any }): Observable<FindOneResult> {
        return new Observable((observer) => {
            let foundId = '';

            (async () => {
                //todo, add filter to limit access to only their shizzle
                ...
                if (item) {
                    ...
                    observer.next({type: 'item', item: classToPlain(classType, item)});
                } else {
                    observer.next({type: 'item', item: undefined});
                    observer.complete();
                }
            })();
    ...
    @Action()
    @Role(RoleType.regular)
    subscribeEntity<T extends IdInterface>(classType: ClassType<T>): Observable<ChannelEntity> {
        return new Observable((observer) => {
            const sub = this.exchange.subscribeEntity(classType, (message) => {
                if (this.needsToBeSend(classType, message.id, message.version)) {
                    ...
                    if (message.type === 'patch') {
                        const copy = clone(message, false, 1);
                        //don't send full item again and again, patch is enough.
                        delete copy['item'];
                        observer.next(copy);
                    } else {
