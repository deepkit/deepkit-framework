...
import { Type, defaultValue, isAutoIncrementType } from '@deepkit/type';
    ...
    template: `
        ...
        >
            ...
            <ng-container *ngIf="editing || (model !== null && model !== undefined)">
                <orm-browser-property-view *ngIf="!editing" [model]="model" [type]="type"></orm-browser-property-view>
                <orm-browser-property-editing
                    *ngIf="editing"
                    (modelChange)="model = $event; modelChange.emit(model)"
                    (done)="editing = false"
                    [type]="type"
                    [model]="model"
                ></orm-browser-property-editing>
        ...
        <div class="actions" *ngIf="!isAutoIncrementType(type) && !editing">
            <dui-icon
                name="clear"
                clickable
                title="Unset"
                (click)="unset(); $event.stopPropagation()"
                [class.active]="!isRequired(type)"
            ></dui-icon>
        </div>
    `,
    styles: [
