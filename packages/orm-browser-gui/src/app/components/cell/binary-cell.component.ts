...
@Component({
    ...
    template: `
        <ng-container *ngIf="image">
            <img [src]="image | objectURL" [openDropdownHover]="drop" style="max-height: 100%; max-width: 50px;" />
        ...
        <dui-icon
            name="download"
            *ngIf="model && model.byteLength > 0"
            (click)="download(); $event.preventDefault(); $event.stopPropagation()"
            clickable
        ></dui-icon>
