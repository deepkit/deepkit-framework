    ...
    template: `
        ...
        <table>
            <tr *ngFor="let kv of state.addedItems | keyvalue">
                ...
                <td>{{ kv.value.length }} new record/s</td>
            </tr>
        </table>
        ...
        <table>
            <tr *ngFor="let kv of state.deletions | keyvalue">
                ...
                <td>delete {{ size(kv.value) }} record/s</td>
            </tr>
        </table>
...
export class DatabaseCommitComponent {
    ...
    stringifyChanges(changes: Changes<any>): string {
        if (changes.$set)
            return Object.entries(changes.$set)
                .map(v => v[0] + '=' + v[1])
                .join(', ');
        if (changes.$unset)
            return Object.entries(changes.$unset)
                .map(v => 'delete ' + v[0])
                .join(', ');
        if (changes.$inc)
            return Object.entries(changes.$inc)
                .map(v => 'increase ' + v[0] + ' by ' + v[1])
                .join(', ');
        return '';
    }
