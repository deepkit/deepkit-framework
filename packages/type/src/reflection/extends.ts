...
function isFunctionLike(type: Type) {
    return (
        ...
        type.kind === ReflectionKind.callSignature ||
        type.kind === ReflectionKind.methodSignature ||
        type.kind === ReflectionKind.objectLiteral ||
        ((type.kind === ReflectionKind.property || type.kind === ReflectionKind.propertySignature) &&
            type.type.kind === ReflectionKind.function)
    );
}

function getFunctionLikeType(
    type: Type,
): TypeMethod | TypeMethodSignature | TypeFunction | TypeCallSignature | undefined {
    if (
        type.kind === ReflectionKind.function ||
        type.kind === ReflectionKind.method ||
        type.kind === ReflectionKind.methodSignature
    ...
    try {
        ...
        if (isFunctionLike(left) && isFunctionLike(right)) {
            ...
            if (leftType && rightType) {
                ...
                if (
                    leftType.kind === ReflectionKind.function &&
                    rightType.kind === ReflectionKind.function &&
                    leftType.function &&
...
function inferFromTuple(left: TypeTuple, right: TypeTuple) {
    ...
    // First, check if there's a rest element in the right pattern and track its position
    let restPosition = -1;
    for (let i = 0; i < right.types.length; i++) {
        if (right.types[i].type.kind === ReflectionKind.rest) {
            restPosition = i;
            break;
        }
    }
    ...
    for (let i = 0; i < right.types.length; i++) {
        ...
        if (
            ...
            // Calculate the index range in left that corresponds to this position in right
            // If there's a rest element before us, we need to extract from the end
            const isAfterRest = restPosition !== -1 && i > restPosition;
            const elementsAfterInRight = right.types.length - i - 1;
            ...
            //for non-rest infer after a rest element, we need to get from the end
            if (rightType.type.kind !== ReflectionKind.rest) {
                if (isAfterRest) {
                    // For patterns like [...infer _, infer L], L should get the last element
                    // The position from the end is: elementsAfterInRight
                    const indexFromEnd = elementsAfterInRight;
                    const targetIndex = inferred.types.length - 1 - indexFromEnd;
                    if (targetIndex >= 0 && targetIndex < inferred.types.length) {
                        inferredType = inferred.types[targetIndex].type;
                    } else {
                        inferredType = { kind: ReflectionKind.never };
                    }
                } else {
