...
type Annotations = any; //actual { [name: symbol]: any[] };, but not support in older TS
export type RuntimeStackEntry = Type | Object | (() => ClassType | Object) | string | number | boolean | bigint;

export type Packed<T = any> = (RuntimeStackEntry | string)[] & { __is?: (data: any) => boolean } & { __type?: Type } & {
    __unpack?: PackStruct;
};
        ...
        function handleUnion(a: Type, unionType: TypeUnion): Type {
            if (a.kind === ReflectionKind.objectLiteral || a.kind === ReflectionKind.class) {
                return unboxUnion({
                    ...
                    types: unionType.types.map(v => collapse(v, a)).filter(v => v.kind !== ReflectionKind.never),
                });
            }
