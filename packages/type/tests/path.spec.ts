...
test('pathResolver object literal', () => {
    ...
    expect(resolver('a')).toMatchObject({
        kind: ReflectionKind.propertySignature,
        type: { kind: ReflectionKind.string },
    });
    expect(resolver('b')).toMatchObject({
        kind: ReflectionKind.propertySignature,
...
test('pathResolver deep ', () => {
    ...
    expect(resolver('b.c')).toMatchObject({
        kind: ReflectionKind.propertySignature,
...
test('pathResolver deep array object', () => {
    ...
    expect(resolver('b.0.0.c')).toMatchObject({
        kind: ReflectionKind.propertySignature,
        type: { kind: ReflectionKind.boolean },
    });
});
...
test('pathResolver deep class', () => {
    ...
    expect(resolver('b')).toMatchObject({
        kind: ReflectionKind.propertySignature,
        type: { kind: ReflectionKind.class, classType: User },
    });
    expect(resolver('b.username')).toMatchObject({
        kind: ReflectionKind.property,
        type: { kind: ReflectionKind.string },
    });
    expect(resolver('b.id')).toMatchObject({ kind: ReflectionKind.property, type: { kind: ReflectionKind.number } });
    expect(resolver('b.settings.0.name')).toMatchObject({
        kind: ReflectionKind.propertySignature,
        type: { kind: ReflectionKind.string },
    });
    expect(resolver('b.settings.0.value')).toMatchObject({
        kind: ReflectionKind.propertySignature,
        type: { kind: ReflectionKind.any },
    });
    expect(resolver('b.settings.1.name')).toMatchObject({
        kind: ReflectionKind.propertySignature,
        type: { kind: ReflectionKind.string },
    });
    expect(resolver('b.settings.1.value')).toMatchObject({
        kind: ReflectionKind.propertySignature,
        type: { kind: ReflectionKind.any },
    });
});
