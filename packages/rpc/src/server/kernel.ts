...
import { ReflectionKind, stringifyUuid, Type } from '@deepkit/type';
...
export class RpcKernelConnection {
    ...
    protected fastReply = new Uint8Array(flagSize + routeParamSize + contextIdSize + actionIdSize);
...
export class RpcKernel {
    ...
    public registerController(controller: ClassType, id?: string | ControllerDefinition<any>, module?: InjectorModule) {
        if (this.autoInjector) {
            if (!this.injector.module.isProvided(controller)) {
                this.injector.module.addProvider({ provide: controller, scope: 'rpc' });
            }
        }
        if (!id) {
        ...
        this.controllers.set('string' === typeof id ? id : id.path, {
            name,
            controller,
            module: module || this.injector.module,
        });
        this.built = false;
    }
