...
test('header', () => {
    ...
    expect(getContextIdOffset(MessageFlag.RouteDirect)).toBe(flagSize + routeParamSize);
    ...
    expect(getActionOffset(MessageFlag.RouteDirect)).toBe(flagSize + routeParamSize);

    expect(getActionOffset(MessageFlag.RouteDirect | MessageFlag.ContextExisting)).toBe(flagSize + routeParamSize + contextIdSize);
    expect(getActionOffset(MessageFlag.RouteDirect | MessageFlag.ContextNew)).toBe(flagSize + routeParamSize);
    expect(getActionOffset(MessageFlag.RouteDirect | MessageFlag.ContextNone)).toBe(flagSize + routeParamSize);
});
...
test('message - direct route - context - ack', () => {
    ...
    expect(isContextFlag(message[0], MessageFlag.ContextExisting)).toBe(true);
    expect(getContextId(message)).toBe(1);
    expect(Buffer.compare(getRouteDirectSrc(message), src)).toBe(0);
    expect(Buffer.compare(getRouteDirectDst(message), dst)).toBe(0);
