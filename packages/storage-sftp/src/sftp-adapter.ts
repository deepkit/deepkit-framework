...
export class StorageSftpAdapter implements StorageAdapter {
    client: Client;
    ...
    async publicUrl(path: string): Promise<string> {
        return `sftp://${this.options.host}:${this.options.port}/${this.getRemotePath(path)}`;
    }
    ...
    async makeDirectory(path: string, visibility: FileVisibility): Promise<void> {
        ...
        await this.client.chmod(this.getRemotePath(path), this.getMode(FileType.Directory, visibility));
    }
    ...
    protected createStorageFile(path: string, fileInfo: FileInfo): StorageFile {
        const file = new StorageFile(path, fileInfo.type !== 'd' ? FileType.File : FileType.Directory);
    ...
    async write(path: string, contents: Uint8Array, visibility: FileVisibility, reporter: Reporter): Promise<void> {
        ...
        await this.client.chmod(this.getRemotePath(path), this.getMode(FileType.File, visibility));
    }
}
