...
test('basic null', () => {
    ...
    expect(deserializeBSON<{ v?: null }>(serialize({ v: undefined }))).toEqual({
        v: undefined,
    });
});
...
test('basic literal', () => {
    expect(deserializeBSON<{ v: 'abc' }>(serialize({ v: null }))).toEqual({
        v: 'abc',
    });
    expect(deserializeBSON<{ v: 'abc' }>(serialize({ v: undefined }))).toEqual({
        v: 'abc',
    });
    expect(deserializeBSON<{ v: 'abc' }>(serialize({}))).toEqual({ v: 'abc' });
    expect(deserializeBSON<{ v: 'abc' }>(serialize({ v: 1234 }))).toEqual({
        v: 'abc',
    });

    expect(deserializeBSON<{ v: 123 }>(serialize({ v: 'abc' }))).toEqual({
        v: 123,
    });
    expect(deserializeBSON<{ v: true }>(serialize({ v: 'abc' }))).toEqual({
        v: true,
    });
});

test('basic optional', () => {
    expect(deserializeBSON<{ v?: string }>(serialize({ v: 'abc' }))).toEqual({
        v: 'abc',
    });
    expect(deserializeBSON<{ v?: string }>(serialize({ v: null }))).toEqual({
        v: undefined,
    });
...
test('basic date', () => {
    const date = new Date();
    expect(deserializeBSON<{ v: Date }>(serialize({ v: date }))).toEqual({
        v: date,
    });
...
test('basic optional property with initializer', () => {
    ...
    expect(deserializeBSON<User>(serialize({ v: new Date(1) }))).toEqual({
        v: new Date(1),
    });
    expect(deserializeBSON<User>(serialize({ v: undefined }))).toEqual({
        v: defaultValue,
    });
    expect(deserializeBSON<User>(serialize({ v: null }))).toEqual({
        v: defaultValue,
    });
...
test('basic signed binary bigint', () => {
    ...
    const obj = { v: 100n };
    const bson = serialize({ v: new Binary(buffer, Binary.SUBTYPE_DEFAULT) });
    const bsonNegative = serialize({
        v: new Binary(Buffer.from([255, 100]), Binary.SUBTYPE_DEFAULT),
    });
    ...
    expect(getBSONDeserializer(undefined, schema)(bson)).toEqual(obj);
    expect(getBSONDeserializer(undefined, schema)(bsonNegative)).toEqual({
        v: -100n,
    });
    expect(getBSONDeserializer(undefined, schema)(serialize({ v: '123' }))).toEqual({ v: 123n });
    expect(getBSONDeserializer(undefined, schema)(serialize({ v: true }))).toEqual({ v: 1n });
...
test('basic union with literals', () => {
    expect(deserializeBSON<{ v: 'a' | 'b' }>(serialize({ v: 'a' }))).toEqual({
        v: 'a',
    });
    ...
    expect(deserializeBSON<{ v: 'a' | 2 }>(serialize({ v: 'a' }))).toEqual({
        v: 'a',
    });
    expect(deserializeBSON<{ v: 'a' | 2 }>(serialize({ v: 'a' }))).toEqual({
        v: 'a',
    ...
    expect(deserializeBSON<{ v: true | number }>(serialize({ v: 2 }))).toEqual({
        v: 2,
    });
...
test('basic uuid', () => {
    const myUuid = uuid();
    expect(deserializeBSON<{ v: UUID }>(serialize({ v: myUuid }))).toEqual({
        v: myUuid,
    });
...
test('basic array', () => {
    const value = ['a', 'b', 'c'];
    expect(deserializeBSON<{ v: string[] }>(serialize({ v: value }))).toEqual({
        v: value,
    });
...
test('basic two array union', () => {
    ...
    expect(deserializeBSON<{ v: MyType }>(serialize({ v: value }))).toEqual({
        v: value,
    });
...
test('tuple', () => {
    {
        type t = { v: [string, number] };
        expect(deserializeBSON<t>(serialize({ v: ['abc', 34] }))).toEqual({
            v: ['abc', 34],
        });
        expect(deserializeBSON<t>(serialize({ v: ['abc', 34, 55] }))).toEqual({
            v: ['abc', 34],
        });
        expect(deserializeBSON<t>(serialize({ v: ['abc', '44'] }))).toEqual({
            v: ['abc', 44],
        });
    }
    {
        ...
        expect(deserializeBSON<t>(serialize({ v: ['44'] }))).toEqual({
            v: [44],
        });
    }
    {
        ...
        expect(deserializeBSON<t>(serialize({ v: ['44', 55] }))).toEqual({
            v: [44, 55],
        });
    }
    {
        type t = { v: [string, ...number[]] };
        expect(deserializeBSON<t>(serialize({ v: [34] }))).toEqual({
            v: ['34'],
        });
        expect(deserializeBSON<t>(serialize({ v: ['44'] }))).toEqual({
            v: ['44'],
        ...
        expect(deserializeBSON<t>(serialize({ v: ['44', 55, 66] }))).toEqual({
            v: ['44', 55, 66],
        });
    }
    {
    ...
    }
    {
        ...
        expect(deserializeBSON<t>(serialize({ v: [true] }))).toEqual({
            v: [true],
        });
        expect(deserializeBSON<t>(serialize({ v: [34, true] }))).toEqual({
            v: ['34', true],
        });
    ...
    }
    {
        ...
        expect(deserializeBSON<t>(serialize({ v: ['abc', true] }))).toEqual({
            v: ['abc', true],
        });
...
test('tuple on union', () => {
...

    {
        ...
        expect(deserializeBSON<t>(serialize({ v: [{ d: true }] }))).toEqual({
            v: [{ d: true }],
        });
        expect(deserializeBSON<t>(serialize({ v: ['abc', 34, 55] }))).toEqual({
            v: ['abc', 34],
...
test('index signature', () => {
    ...
    expect(deserializeBSON<A>(serialize({ a: 1, b: 2 }))).toEqual({
        a: 1,
        b: 2,
    });
});
...
test('index signature in union', () => {
    ...
    expect(deserializeBSON<t>(serialize({ another: true }))).toEqual({
        another: true,
    });
    expect(deserializeBSON<t>(serialize({ a: 1, b: 2 }))).toEqual({
        a: 1,
        b: 2,
    });
});
...
test('index signature template literal', () => {
    ...
    expect(deserializeBSON<A>(serialize({ a23: 1, a3: 3 }))).toEqual({
        a23: 1,
        a3: 3,
    });
});
...
test('index signature multiple', () => {
    ...
    expect(deserializeBSON<A>(serialize({ a23: 2, 12: true }))).toEqual({
        a23: 2,
        12: true,
    });
});
...
test('index signature + object literal', () => {
    ...
    expect(deserializeBSON<t>(serialize({ abc: 'yes' }))).toEqual({
        abc: 'yes',
    });
    expect(() => deserializeBSON<t>(serialize({ a23: 1 }))).toThrow('Cannot convert undefined value to string');
    expect(deserializeBSON<t>(serialize({ abc: 'yes', 12: true }))).toEqual({
        abc: 'yes',
        12: true,
    });
...
test('circular', () => {
    ...
    const bson = serializeBSONWithoutOptimiser({
        items: [{ id: 0, child: { id: 2 } }],
    } as Response);
