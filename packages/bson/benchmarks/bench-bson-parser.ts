...
suite.add('deepkit/type JS JIT', () => {
    const items = parser(bson);
});

suite.add('deepkit/type JS generic ParserV1', () => {
    const items = parseObject(new BaseParser(bson));
});

suite.add('deepkit/type JS generic ParserV2', () => {
    const items = parseObject(new ParserV2(bson));
});

suite.add('deepkit/type JS generic ParserV3', () => {
...
suite.add('deepkit/type JIT 1 item', () => {
