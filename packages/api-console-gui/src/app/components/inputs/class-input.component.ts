...
import { ReflectionClass, TypeClass, TypeObjectLiteral, isBackReferenceType, isReferenceType } from '@deepkit/type';
...
@Component({
    template: `
        <ng-container *ngIf="!schema"> No schema </ng-container>

        <div class="box children" *ngIf="schema">
            <dui-select
                textured
                style="width: 100%; margin-bottom: 5px;"
                [(ngModel)]="model.asReference"
                *ngIf="isReferenceLike(type)"
            ...
            <div style="padding: 4px;" *ngIf="showOnlyPrimaryKey && schema.getPrimary() as p">
                <api-console-input
                    [optional]="false"
                    [model]="model.getProperty(p.name)"
                    [type]="p.property"
