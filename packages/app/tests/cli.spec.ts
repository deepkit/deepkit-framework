...
test('parser', async () => {
    ...
    expect(parseCliArgs(['1234', '--help'])).toEqual({
        '@': ['1234'],
        help: true,
    });
    expect(parseCliArgs(['1234', '--help', '--verbose'])).toEqual({
        '@': ['1234'],
        help: true,
        verbose: true,
    });

    expect(parseCliArgs(['1234', 'abc', '--id', '1'])).toEqual({
        '@': ['1234', 'abc'],
        id: '1',
    });
    expect(parseCliArgs(['1234', 'abc', '--id 1'])).toEqual({
    ...
    expect(parseCliArgs(['1234', 'abc', '--id 1', '--id 2'])).toEqual({
        '@': ['1234', 'abc'],
        id: ['1', '2'],
    });
});
...
For more information on a specific command or topic, type '[command/topic] --help'`,
    ...
    expect(await app(['test', 'Peter'])).toEqual({
        code: 0,
        output: 'Hello Peter',
    });
    expect(await app(['test2', 'Peter', 'Germany'])).toEqual({
        code: 0,
        output: 'Hello Peter from Germany!',
    });
    expect(await app(['add-user', 'Peter', '--group', '2'])).toEqual({
        code: 0,
        output: 'Added user Peter to group 2',
    });
    ...
    expect(await app(['del-user-by-id', '123'])).toEqual({
        code: 0,
...
test('executeCommand 2', async () => {
    ...
    expect(await app(['show-user', 'Peter'])).toEqual({
        code: 0,
        output: 'showUser Peter false',
    });
    expect(await app(['show-user', 'Peter', '--detail'])).toEqual({
        code: 0,
        output: 'showUser Peter true',
    });

    expect(await app(['show-user2', 'Peter'])).toEqual({
        code: 0,
        output: 'showUser2 Peter default',
    });
    expect(await app(['show-user2', 'Peter', '--view', 'json'])).toEqual({
        code: 0,
    ...
    expect(await app(['show-users', '--user', 'Peter'])).toEqual({
        code: 0,
        output: 'showUsers Peter',
    });
    expect(await app(['show-users', '--user', 'Peter', '--user', 'John'])).toEqual({
        code: 0,
        output: 'showUsers Peter,John',
    });
});
...
test('executeCommand 3', async () => {
    ...
    expect(await app(['show-user', '--name', 'Peter'])).toEqual({
        code: 0,
        output: 'showUser Peter',
    });
    expect(await app(['show-user', '-n', 'Peter'])).toEqual({
        code: 0,
        output: 'showUser Peter',
    });
...
test('object literal flag', async () => {
    ...
    expect(await app(['show-user', '--name', 'Peter'])).toEqual({
        code: 0,
        output: 'name=Peter age=undefined',
    });
    expect(await app(['show-user', '--name', 'Peter', '--age', '32'])).toEqual({
        code: 0,
        output: 'name=Peter age=32',
    });
});
