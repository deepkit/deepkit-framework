...
// Configure via environment variables or use defaults for docker compose (MinIO)
// docker compose up -d  (uses port 10900)
const S3_ENDPOINT = process.env.S3_ENDPOINT || 'http://localhost:10900';
const S3_REGION = process.env.S3_REGION || 'us-east-1';
const S3_BUCKET = process.env.S3_BUCKET || 'deepkit-test';
const S3_ACCESS_KEY = process.env.S3_ACCESS_KEY || process.env.AWS_ACCESS_KEY_ID || 'minioadmin';
const S3_SECRET_KEY = process.env.S3_SECRET_KEY || process.env.AWS_SECRET_ACCESS_KEY || 'minioadmin';
...
test('s3 url', async () => {
    // Skip URL test when using MinIO (different URL format)
    if (S3_ENDPOINT.includes('localhost')) return;
