...
export class HttpListener {
    ...
    async onRoute(event: typeof httpWorkflow.onRoute.event) {
        ...
        try {
            ...
            if (resolved) {
                ...
                // If middleware already sent the response, skip to response state for logging/cleanup
                if (event.sent) {
                    event.next(
                        'response',
    ...
    onParametersFailed(event: typeof httpWorkflow.onParametersFailed.event): void {
        ...
        if (event.error instanceof SerializationError) {
            ...
            this.logger.error(
                `Controller for route ${event.route.getFullPath()} parameter resolving error:`,
                event.error,
            );
