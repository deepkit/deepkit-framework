...
export class FrameworkModule extends createModuleClass({
    ...
    listeners: [DatabaseListener, BrokerListener],
    controllers: [
    ...
    process() {
        ...
        if (this.config.publicDir) {
            const localPublicDir = isAbsolute(this.config.publicDir)
        ...
        this.configureProvider<Stopwatch>(
            stopwatch => {
    ...
    protected setupDatabase() {
        for (const db of this.dbs) {
            ...
            db.module.configureProvider(
                (db: Database, eventDispatcher: EventDispatcher, logger: Logger, stopwatch: Stopwatch) => {
    ...
    processController(module: AppModule<any>, config: ControllerConfig) {
        ...
        if (this.rpcControllers.controllers.has(rpcConfig.getPath())) {
            throw new DeepkitError(
                'DK-F009',
                `Already an RPC controller with the name ${rpcConfig.getPath()} registered.`,
            );
        }
