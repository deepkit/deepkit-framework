...
test('test required', async () => {
  ...
  expect(await validate(Model, { name: 'foo', map: true })).toEqual([
    { message: 'Invalid type. Expected object, but got boolean', path: 'map' },
  ]);
  expect(await validate(Model, { name: 'foo', array: 233 })).toEqual([
    { message: 'Invalid type. Expected array, but got number', path: 'array' },
  ]);
...
test('test deep', async () => {
  ...
  expect(await validate(Model, instance)).toBeArrayOfSize(1);
  expect(await validate(Model, instance)).toEqual([
    { message: 'Required value is undefined', path: 'deep' },
  ]);

  instance.deep = new Deep();
  expect(await validate(Model, instance)).toEqual([
    { message: 'Required value is undefined', path: 'deep.name' },
  ]);

  instance.deep.name = 'defined';
  instance.deepArray.push(new Deep());
  expect(await validate(Model, instance)).toEqual([
    { message: 'Required value is undefined', path: 'deepArray.0.name' },
  ]);

  instance.deepArray[0].name = 'defined';
  instance.deepMap.foo = new Deep();
  expect(await validate(Model, instance)).toEqual([
    { message: 'Required value is undefined', path: 'deepMap.foo.name' },
  ]);
...
test('test string', async () => {
  ...
  expect(await validate(Model, { id: 2 })).toEqual([
    { message: 'No String given', path: 'id' },
  ]);
  expect(await validate(Model, { id: null })).toEqual([
    { message: 'No String given', path: 'id' },
  ]);
  ...
  expect(await validate(ModelOptional, { id: null })).toEqual([
    { message: 'No String given', path: 'id' },
  ]);
...
test('test number', async () => {
  ...
  expect(await validate(Model, { id: '3' })).toEqual([]);
  expect(await validate(Model, { id: 'a' })).toEqual([
    { message: 'No Number given', path: 'id' },
  ]);
  expect(await validate(Model, { id: null })).toEqual([
    { message: 'No Number given', path: 'id' },
  ]);
  ...
  expect(await validate(ModelOptional, { id: '3' })).toEqual([]);
  expect(await validate(ModelOptional, { id: 'a' })).toEqual([
    { message: 'No Number given', path: 'id' },
  ]);
  expect(await validate(ModelOptional, { id: null })).toEqual([
    { message: 'No Number given', path: 'id' },
