      ...
      if (!(value instanceof typeValue)) {
        throw new Error(
          `Could not convert ${getClassPropertyName(
            ...
            `class instance of ${getClassName(typeValue)}. Got ${getClassName(
              value
            )}`
        );
      }
...
export function isArrayType<T>(
  ...
  return (
    getCachedMetaData('marshal:isArray', classType.prototype, property) || false
  );
}
...
export function isMapType<T>(
  ...
  return (
    getCachedMetaData('marshal:isMap', classType.prototype, property) || false
  );
}
