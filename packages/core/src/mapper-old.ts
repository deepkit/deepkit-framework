...
export function classToPlain<T>(classType: ClassType<T>, target: T, options?: { excludeReferences?: boolean }): any {
    ...
    for (const property of getClassSchema(classType).getClassProperties().values()) {
 ...
 * @hidden
 */
export function findParent<T>(parents: any[], parentType: ClassType<T>): T | null {
...
export function toClass<T>(
    ...
    for (const propertyName of parameterNames) {
        const property = schema.classProperties.get(propertyName)!;
