...
import { ChangeFeed, FrameItem, FrameParser, ViewState, formatTime } from './frame';
...
class TimelineContainer extends Container {
    ...
    matchesFilter(item: FrameItem): boolean {
        ...
        if (
            this.filterQuery &&
            this.filterQuery[0] !== '*' &&
            !item.frame.label.toLowerCase().startsWith(this.filterQuery)
        )
            return false;
        if (
            this.filterQuery &&
            this.filterQuery[0] === '*' &&
            !item.frame.label.toLowerCase().includes(this.filterQuery.slice(1))
        )
            return false;
