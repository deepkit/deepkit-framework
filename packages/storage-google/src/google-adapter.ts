import { FileType, FileVisibility, pathDirectory, pathNormalize, Reporter, resolveStoragePath, StorageAdapter, StorageFile } from '@deepkit/storage';
import { Bucket, File, Storage, StorageOptions } from '@google-cloud/storage';
import { fixAsyncOperation } from '@deepkit/core';

export interface StorageGoogleOptions extends StorageOptions {
    bucket: string;
    path: string;
}

export class StorageGoogleAdapter implements StorageAdapter {
    client: Storage;
    bucket: Bucket;
    options: StorageGoogleOptions = {
        bucket: '',
        path: '',
    };

    constructor(options: Partial<StorageGoogleOptions> = {}) {
    ...
    protected getRemotePath(path: string): string {
        ...
        return resolveStoragePath([this.options.path, path]).slice(1); //no leading slash
    }
    ...
    async getFiles(path: string, recursive: boolean = false): Promise<StorageFile[]> {
        ...
        for (const folder of prefixes) {
            ...
            result.push(new StorageFile(this.mapRemotePathToLocal(folder), FileType.Directory));
        }

        for (const file of files[0]) {
            if (prefixes.includes(file.name)) continue;
            if (file.name === dir) continue;
            result.push(this.createStorageFile(this.mapRemotePathToLocal(file.name), file));
        }
    ...
    async get(path: string): Promise<StorageFile | undefined> {
        ...
        const file = this.createStorageFile(path, first);
    ...
    protected createStorageFile(path: string, fileInfo: File): StorageFile {
        //if path is a directory, we have in first.name trailing slash
        const type = fileInfo.name.endsWith('/') ? FileType.Directory : FileType.File;
        const file = new StorageFile(path, type);
