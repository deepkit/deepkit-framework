...
@Component({
    ...
    template: `
        <div class="app-content-full normalize-text">
            ...
            <textarea
                *ngIf="(question && question.allowEdit) || ask"
                (keyup)="onKeyUp($event)"
...
export class CommunityQuestionComponent implements OnInit {
    ...
    async prompt() {
        ...
        try {
            const question = await this.client.main.createQuestion(
                text,
                this.question?.id,
                this.getAuthId(this.question?.id),
            );
            ...
            response.subscribe(next => {
                ...
                if (next.remove)
                    messageToWrite.content.children = messageToWrite.content.children.slice(0, -next.remove);
